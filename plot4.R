...
plot4 <- function() {
       ...
       # Adding Date column + weekday column from Lubridate and the filter	clause for the date
        ...
        #global active power
        plot1 <- ggplot(data=EPC.2, aes(x=Date.Time, y=as.numeric(Global_active_power), group=1)) + geom_line() + ylab("Global Active Power (kilowatts)")  + scale_x_datetime(breaks = date_breaks("1 day"),  labels = date_format("%a")) + xlab(" ") 
        
        # Voltage
        plot2 <- ggplot(data=EPC.2, aes(x=Date.Time, y=as.numeric(Voltage), group=1)) + geom_line() + ylab("Voltage")  + scale_x_datetime(breaks = date_breaks("1 day"),  labels = date_format("%a")) + xlab(" ")
        
        # sub metering
        plot3 <- ggplot(data=EPC.2, aes(x=Date.Time)) + 
        ...
        plot4 <- ggplot(data=EPC.2, aes(x=Date.Time, y=as.numeric(Global_reactive_power), group=1)) + geom_line() + ylab("Global Reactive Power")  + scale_x_datetime(breaks = date_breaks("1 day"),  labels = date_format("%a")) + xlab(" ")
